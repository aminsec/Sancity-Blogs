...
function LikedPosts(){
    ...
    useEffect(() => {
        const grabData = async () => {
            const req = await fetch("/api/v1/user/likes", {
                method: "GET",
                credentials: "include"
            })
            ...
            if(req.ok){
                ...
                if(data.state == "success"){
                    const blogsId = data.blogs_id;
                    for(let id of blogsId){
    ...
    function Blogs(){
        if(userBlogs.length > 0){
            return(
                <>
                    {userBlogs.map((blog) => (
                        <div className='blogsFav' onClick={() => {navigate(`/blogs/${blog.blog_id}`)}}>
                            ...
                            <p id='blogs-username'><a className='likedBlogs-blog-username-a' href={`/writer/${blog.user.userid}`} onClick={(e) => {e.stopPropagation();}}>{blog.user.username}</a></p>
            ...
            return(
                <div id="noBlogs"><p>You have not liked anything yet</p></div>
            )
        }
    }
    return(
        <>
        <h1 id="titles">Liked Blogs ❤️</h1>
        <hr id="line"></hr>
        <Blogs />
        ...
        <div id="left-panel">
            ...
            <img id="likedBlogs-pen" src="/statics/img/feather.png" onClick={() => {navigate("/myaccount/new")}}></img>
            <img id="likedBlogs-setting" src="/statics/img/setting.png" onClick={() => {navigate("/myaccount/setting")}}></img>
            ...
            <li className="panelElements" id="elements-likes" onClick={() => {navigate("/myaccount/likes")}}>Liked Blogs</li>
